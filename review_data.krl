ruleset review_data {
  ...
  global {    
  	get_review = function (term, location) {
  		...
  		{ 	"term" : term,
   		...
   		}).pick("$.content.businesses[0]").decode();//.pick("$.content)$.businesses[0]").decode();
  	};
  }
  ...
  rule location_show is active {
    	...
    	pre {
    		...
    		img_url = r.pick("$.rating_img_url").encode();
  			review_count = r.pick("$.review_count").encode();
  			name = r.pick("$.name").encode();
  			avg_rating = r.pick("$.avg_rating").encode();
  			review = r.pick("$.reviews[0].text_excerpt").encode();
       	 my_html = <<
          <div id=review>
          #{r.encode()}
          	Review:
